...
echo "##########################################" 1>&2
echo -n "Running initiate_request_enumurator: " 1>&2
date 1>&2
python3 $BASE_PATH/initiate_request_enumurator.py > initiate_request_enumurator.log 2>&1
STATUS=$?
echo -n "Finished running initiate_request_enumurator: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run initiate_request_enumurator." 1>&2
  cat initiate_request_enumurator.log 1>&2
  echo "{}"
  exit $STATUS
fi
