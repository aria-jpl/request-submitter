...
echo "##########################################" 1>&2
echo -n "Running initiate_submit_s1gunw: " 1>&2
date 1>&2
python3 $BASE_PATH/initiate_submit_s1gunw.py > initiate_submit_s1gunw.log 2>&1
STATUS=$?
echo -n "Finished running initiate_submit_s1gunw: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run initiate_submit_s1gunw." 1>&2
  cat initiate_submit_s1gunw.log 1>&2
  echo "{}"
  exit $STATUS
fi
